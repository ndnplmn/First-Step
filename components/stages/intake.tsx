...
// 8: supportNetwork      (split from step 8 — reduces cognitive overload)
// 9: previousTherapy + medication (clinical context — both optional)
...
function getStepAnswer(
  ...
  switch (step) {
    ...
    case 8: {
      ...
      return !values.hasSupportNetwork ? t('intake.no')
...
export function Intake({ onComplete, onBack }: IntakeProps) {
  ...
  const buildPatient = async (): Promise<Patient> => {
    ...
    return {
      ...
      hasSupportNetwork: values.hasSupportNetwork ?? false,
      supportDescription:
