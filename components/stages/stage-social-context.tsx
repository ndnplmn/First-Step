...
export function StageSocialContext({ session, patient, priorSessions = [], onAdvance, onUpdate: _onUpdate }: Props) {
  ...
  const lastExploration = [...priorSessions]
    .reverse()
    .find(s => s.explorationRecord?.framework === 'adleriano')?.explorationRecord
  ...
  const handleFinalAdvance = async () => {
    ...
    try {
      ...
      const record = await synthesizeExploration({
        ...
        frameworkKey: 'adleriano',
        frameworkName: session.frameworkMatches[0]?.name ?? 'Psicología Individual de Adler',
      ...
      onAdvance({ sessionNumber: session.sessionNumber, framework: 'adleriano', frameworkName: session.frameworkMatches[0]?.name ?? 'Psicología Individual de Adler', stage3Type: 'social_context', insights: [], aiReflection: '', completedAt: Date.now() });
    }
  };
